...
def piano_port_setup():
    ...
    try:
        ...
        midi_out = rtmidi_MidiOut()
        out_avaliable_ports = midi_out.get_ports()

        closes_match_out_port = difflib.get_close_matches('LoopBe Internal MIDI',out_avaliable_ports)
        midi_out.open_port(out_avaliable_ports.index(closes_match_out_port))
        return 1

    except:
        print("Piano and LoopBe Internal Port Setup Failure")
        midi_in = []
        midi_out = []
        return 0
...
def complete_tutor():
    ...
    cwd_path = os.path.dirname(os.path.abspath(__file__))
    file = setting_read('midi_file_location')
    #print('file: ' + file)

    # Obtaining the MIDI sequence
    midi_setup(file, cwd_path)
