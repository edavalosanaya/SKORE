...
def midi_to_note_event_info(midi_file):
    ...
    for track in pattern:
        for event in track:
            if isinstance(event, NoteEvent):
                if event.tick > 0:
                    note_event_matrix.append('DELAY,'+str(event.tick))
                if isinstance(event, NoteOffEvent):
                    note_event_matrix.append('OFF,'+str(event.pitch))
                else:
                    note_event_matrix.append('ON,'+str(event.pitch))
